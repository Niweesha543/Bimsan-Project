  ...
  return (
    <section className="py-12 px-4 md:px-16 bg-white relative">
      <div className="flex items-center justify-center gap-4 md:gap-6">
        <button 
          onClick={handlePrev} 
          className="text-2xl md:text-3xl font-bold text-gray-600 hover:text-orange-500 transition"
        >
          &lt;
        </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 w-full max-w-6xl">
          {visiblePackages.map((pkg, index) => (
            <div
              ...
              className="cursor-pointer w-full max-w-sm mx-auto h-[450px] flex flex-col rounded-md shadow-md border border-gray-200 overflow-hidden transform transition hover:scale-105 hover:shadow-xl"
            >
              ...
              <div className="p-4 flex flex-col justify-between flex-1 space-y-1">
                ...
                <h3 className="text-xl font-semibold text-blue-900 font-serif">{pkg.title}</h3>
                <p className="text-gray-600 font-sans text-base flex-1">{pkg.description}</p>
              </div>
            </div>
          ))}
        </div>
